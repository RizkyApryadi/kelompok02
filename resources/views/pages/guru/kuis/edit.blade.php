            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="quiz_title" name="quiz_title"
                        value="{{ old('quiz_title', $quiz->quiz_title) }}" required>
                </div>
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="quiz_description" name="quiz_description" rows="3"
                        required>{{ old('quiz_description', $quiz->quiz_description) }}</textarea>
                </div>
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="quiz_duration" name="quiz_duration"
                        value="{{ old('quiz_duration', $quiz->quiz_duration) }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="target_class" name="target_class" class="form-control" required>
                        ...
                        <option value="{{ $kelas->id }}" {{ $quiz->target_class == $kelas->id ? 'selected' : '' }}>
                            {{ $kelas->nama_kelas }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3 mt-3">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="show_answers" value="0"
                            {{ !$quiz->show_answers ? 'checked' : '' }}>
        ...
        <div id="question-container">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    @foreach(['A','B','C','D'] as $optIndex => $opt)
                    <div class="mb-2">
                        <input type="text" class="form-control" name="questions[{{ $index }}][options][]"
                            value="{{ $question->options[$opt] ?? '' }}" placeholder="Pilihan {{ $opt }}" required>
                    </div>
                    @endforeach
                    <div class="mb-3">
                        <label class="form-label">Kunci Jawaban</label>
                        <select name="questions[{{ $index }}][answer]" class="form-control" required>
                            <option value="">-- Pilih Kunci Jawaban --</option>
                            @foreach(['A','B','C','D'] as $opt)
                            <option value="{{ $opt }}" {{ $question->answer == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
